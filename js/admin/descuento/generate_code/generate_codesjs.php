...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar arrays despuÃ©s de obtener los elementos del DOM
    ...
    // Inicializar sidebar (soluciÃ³n para el error setupSidebarCollapse)
    if (typeof setupSidebarCollapse === 'function') {
        setupSidebarCollapse();
    }

    // Si estamos en modo ediciÃ³n, inicializar campos
    ...
    // FunciÃ³n para inicializar modo ediciÃ³n
    function initializeEditMode() {
        const codeData = window.editCodeData;
        console.log('Inicializando modo ediciÃ³n con datos:', codeData);
        
        // Forzar la actualizaciÃ³n de campos segÃºn tipo de descuento
        setTimeout(() => {
            updateDiscountFields();
            updateDiscountFieldsOriginal();
            
            // Si hay productos seleccionados, mostrar la secciÃ³n
    ...
    // FunciÃ³n para cargar usuarios desde el servidor
        ...
        fetch('<?= BASE_URL ?>/admin/api/descuento/search_users.php')
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    allUsers = data;
                    filteredUsers = data;
                    console.log('Usuarios cargados vÃ­a AJAX:', allUsers.length);
                } else {
    ...
    // FunciÃ³n para limpiar la selecciÃ³n de productos
    function clearProductsSelection(skipVisibilityUpdate = false) {
        ...
        // Solo actualizar visibilidad si no se estÃ¡ saltando (evita recursiÃ³n)
        if (!skipVisibilityUpdate) {
            updateProductsSelectionVisibility();
        }

        // Actualizar checkboxes en el modal si estÃ¡ abierto
    ...
    // FunciÃ³n para actualizar la visibilidad del resumen de productos segÃºn "Aplicar a todos"
    function updateProductsSelectionVisibility() {
        ...
        if (applyToAllCheckbox && selectedProductsInfo) {
            if (applyToAllCheckbox.checked) {
                // Si aplica a todos, ocultar el resumen y limpiar selecciÃ³n
                selectedProductsInfo.style.display = 'none';
                // Pasar true para evitar recursiÃ³n infinita
        ...
        // TambiÃ©n actualizar visibilidad del botÃ³n del modal
        if (openProductsModalBtn) {
            openProductsModalBtn.style.display = (applyToAllCheckbox && applyToAllCheckbox.checked) ? 'none' : 'block';
        }
    }

    // FunciÃ³n para actualizar el resumen de productos seleccionados
    function updateProductsSelectionSummary() {
        ...
        if (selectedProductsInfo && selectedProductsCountDisplay) {
            // Solo mostrar si NO estÃ¡ marcado "Aplicar a todos" Y hay productos seleccionados
    ...
    // FunciÃ³n para cargar productos reales desde la base de datos
    function loadRealProducts(searchTerm = '') {
        ...
        // Hacer peticiÃ³n AJAX para obtener productos reales
        ...
        fetch('<?= BASE_URL ?>/admin/api/descuento/get_products.php', {
            ...
            .catch(error => {
                ...
                if (typeof showAlert === 'function') {
                    showAlert('Error de conexiÃ³n al cargar productos: ' + error.message, 'error');
                }
            })
            .finally(() => {
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <div>
                        ...
                        <div style="font-size: 0.8em; color: #666;">
                            ${escapeHtml(product.category)} â€¢ ${genderText}
                        </div>
                        <div style="font-size: 0.7em; color: #888;">
                            ${product.color_variants} colores â€¢ ${product.size_variants} tallas
                        </div>
                    </div>
                </div>
            </td>
    ...
    // FunciÃ³n para limpiar la selecciÃ³n de usuarios
    ...
    // FunciÃ³n para actualizar el resumen de usuarios seleccionados
    function updateUsersSelectionSummary() {
        ...
        if (selectedUsersInfo && selectedUsersCountDisplay) {
            // Solo mostrar si estÃ¡ marcado "Enviar notificaciÃ³n" Y hay usuarios seleccionados
    ...
    function loadUsers(searchTerm = '') {
        console.log('Cargando usuarios con tÃ©rmino:', searchTerm);
    ...
    // FunciÃ³n para actualizar los campos segÃºn el tipo de descuento
    function updateDiscountFields() {
        ...
        switch(type) {
            ...
            case '2': // Monto fijo
                ...
                break;
            case '3': // EnvÃ­o gratis
                if (shippingMethodGroup) shippingMethodGroup.style.display = 'block';
                break;
        }
    }

    // Mostrar/ocultar campos segÃºn tipo de descuento (versiÃ³n original mejorada)
    ...
    function updateDiscountFieldsOriginal() {
        ...
        // En modo ediciÃ³n, no limpiar valores
        if (!window.editMode) {
            // Limpiar valores de campos que se ocultan solo si NO estamos en modo ediciÃ³n
        ...
        // Mostrar los campos relevantes segÃºn el tipo
        switch (discountType) {
            case '1': // Porcentaje
    ...
    // Manejar cambio en "Enviar notificaciÃ³n"
    if (sendNotification) {
        sendNotification.addEventListener('change', function() {
            ...
            // Si se desmarca la notificaciÃ³n, limpiar la selecciÃ³n de usuarios
            if (!isChecked) {
                clearUsersSelection();
            } else {
    ...
    // Manejar eliminación de códigos
    document.querySelectorAll('.btn-delete-row').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            if (modal) {
                // Actualizar ID actual para eliminación
                const confirmBtn = document.getElementById('delete-modal-confirm');
                if (confirmBtn) {
                    modal.style.display = 'flex';
                    setTimeout(() => modal.classList.add('active'), 10);
                    
                    // Actualizar el manejador del botón confirmar
                    const newConfirmBtn = confirmBtn.cloneNode(true);
                    confirmBtn.parentNode.replaceChild(newConfirmBtn, confirmBtn);
                    
                    newConfirmBtn.addEventListener('click', function() {
                        window.location.href = `generate_codes.php?action=delete&id=${id}`;
                    });
                }
            } else if (confirm('Estás seguro de que deseas eliminar este código?')) {
