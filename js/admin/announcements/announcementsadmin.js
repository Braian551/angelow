document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadAnnouncements(page = 1) {
        ...
        fetch(`${window.BASE_URL}/ajax/admin/get_announcements.php?${params}`, {
    ...
    function displayAnnouncements(announcements) {
        ...
        container.innerHTML = announcements.map(announcement => {
            ...
            const imageHtml = announcement.image
                ? `<img src="${window.BASE_URL}/${announcement.image}" alt="${announcement.title}" style="max-width: 100%; height: auto; border-radius: 4px;">`
                ...
                <div class="product-card">
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-actions" style="margin-top: 15px; display: flex; gap: 8px;">
                            <a href="${window.BASE_URL}/admin/announcements/edit.php?id=${announcement.id}" class="btn btn-sm btn-primary">
    ...
    function updateResultsCount(total) {
        ...
        // Ocultar botón "Agregar" si ya hay 2 anuncios
        const addBtn = document.getElementById('add-announcement-btn');
        if (addBtn) {
            if (total >= 2) {
                addBtn.style.display = 'none';
                // Mostrar mensaje informativo si no existe
                if (!document.getElementById('max-announcements-msg')) {
                    const msg = document.createElement('div');
                    msg.id = 'max-announcements-msg';
                    msg.className = 'alert alert-info';
                    msg.style.marginTop = '10px';
                    msg.innerHTML = '<i class="fas fa-info-circle"></i> Máximo de anuncios alcanzado (2). Elimina uno existente para agregar otro.';
                    resultsCount.parentElement.appendChild(msg);
                }
            } else {
                addBtn.style.display = '';
                const existingMsg = document.getElementById('max-announcements-msg');
                if (existingMsg) {
                    existingMsg.remove();
                }
            }
