...
<body>
    ...
    <!-- Scripts -->
    <script>
        window.orderStatuses = <?= json_encode([
            'pending' => 'Pendiente',
            'processing' => 'En proceso',
            'shipped' => 'Enviado',
            'delivered' => 'Entregado',
        ...
        window.paymentStatuses = <?= json_encode([
            'pending' => 'Pendiente',
            'paid' => 'Pagado',
            'failed' => 'Fallido',
            'refunded' => 'Reembolsado',
            ...
            .then(response => {
                ...
                if (!response.ok) {
                    if (contentType.includes('application/json')) {
                        return response.json().then(d => { throw new Error(d.message || d.error || 'Error al actualizar estado'); });
                    }
                    return response.text().then(t => { throw new Error(t || 'Error al actualizar estado'); });
                }
                if (!contentType.includes('application/json')) {
                    return response.text().then(t => { throw new Error('Respuesta inesperada: ' + (t || '')); });
                }
